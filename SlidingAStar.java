...
//This program solves sliding puzzle using A* Heuristic
public class SlidingAStar
{
  ...
  public SlidingAStar(int[][] initial, int[][] goal, int sizeIn, PrintWriter PWOut)
  {
  ...
  private int find(Board board, LinkedList<Board> list)
  {
	  for (int i = 0; i < list.size(); i++)		//compare board with each
		if (identical(board, list.get(i)))		//element of a list
			return i;
